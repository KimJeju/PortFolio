$(function(){
    ...
    /*좌우 슬라이드*/
    $('#imgslide>a').eq(0).siblings().css({left:'-2000px'});
    var slideIndex=0;
    setInterval(function(){
        if(slideIndex<3){slideIndex++;}
        else{slideIndex=0;}
        $('#imgslide>a').eq(slideIndex).siblings().animate({left:'-2000px'},400);
        $('#imgslide>a').eq(slideIndex).animate({left:'0px'},400);
    },3000);
    /*좌우 슬라이드*/
    ...
    /*상하 슬라이드*/
    $('#imgslide2>a').eq(0).siblings().css({top:'-500px'});
    var slideIndexx=0;
    setInterval(function(){
